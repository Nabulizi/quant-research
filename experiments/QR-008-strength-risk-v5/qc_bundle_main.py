# GENERATED QR-008 bundle (quant-research@cc6c6cb); edit only SMOKE:
...
def numeric_field(field, period='one_year'):
...
def equal_weight_return(members, previous, current):
 ...
 for member in members:
  ...
  if not finite_number(old_price) or not finite_number(new_price) or old_price <= 0 or (new_price < 0):
   missing.append(member)
   continue
  observed.append(new_price / old_price - 1)
 if missing:
  raise ValueError(f'missing a valid holding-period return for {len(missing)} of {len(members)} members')
...
class QR008StrengthRiskV5(QCAlgorithm):
 ...
 def _step(self):
  ...
  if not first:
   ...
   self._spy_series.append(spy_p / self._spy_prev - 1 if spy_p is not None and self._spy_prev is not None else None)
 ...
 def _score_universe(self):
  ...
  record = {'breadth': len(strong), 'insufficient': n_insufficient, 'suspect': n_suspect, 'miss': miss, 'cand': sorted((s.value for s in strong)), 'cand_buckets': cand_industries}
