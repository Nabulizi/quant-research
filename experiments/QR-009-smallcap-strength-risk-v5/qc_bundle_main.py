...
def return_stats(returns, periods_per_year=12):
 if len(returns) < 2:
...
def equal_weight_turnover(old_members, new_members):
 ...
 absolute_change = sum((abs((new_weight if name in new_names else 0.0) - (old_weight if name in old_names else 0.0)) for name in names))
 return absolute_change / 2.0

def apply_cost(gross_return, one_way_turnover, bps_per_side):
 ...
 return gross_return - 2 * one_way_turnover * bps_per_side / 10000.0
import math
import re
...
_JS_MAX_VALUE = 1.7976931348623157e+308
CRITERION_WEIGHT = {'earningsQuality': 3, 'leverage': 3, 'revenueGrowth': 2, 'revenueAcceleration': 2, 'fcfYieldLevel': 2, 'marginInflection': 2, 'peCompression': 2, 'valuation': 1, 'dividendCoverage': 1, 'pricePosition': 1, 'ytdMomentum': 1, 'dividendYield': 1}
CRITERION_KEYS = list(CRITERION_WEIGHT)
_CYCLICAL_PATTERNS = ['semiconductor', 'automobile', '\\bautos?\\b', 'auto components', '\\boil\\b', '\\bgas\\b', '\\bcoal\\b', 'petroleum', 'drilling', 'energy equipment', 'consumable fuels', 'mining', 'metals', '\\bsteel\\b', 'aluminum', 'chemical', '\\bmarine\\b', 'shipping', 'airline', 'construction', 'building', 'homebuild', 'paper', 'forest']
_FINANCIAL_PATTERNS = ['financ', '\\bbank', 'insurance', 'capital markets']
_REIT_PATTERNS = ['\\breits?\\b', 'real estate']
_OVERRIDE_BALANCE_SHEET = {'COF', 'DFS', 'AXP', 'SYF', 'ALLY'}
_OVERRIDE_ASSET_LIGHT = {'BLK', 'TROW', 'BEN', 'CME', 'ICE', 'NDAQ', 'CBOE', 'SPGI', 'MCO', 'MSCI', 'FDS', 'MORN', 'V', 'MA'}

def _matches(industry, patterns):
 return isinstance(industry, str) and any((re.search(p, industry, re.IGNORECASE) for p in patterns))

def is_cyclical_industry(industry):
 return _matches(industry, _CYCLICAL_PATTERNS)

def is_financial_industry(industry):
 return _matches(industry, _FINANCIAL_PATTERNS)

def is_reit_industry(industry):
 return _matches(industry, _REIT_PATTERNS)

def classify_financial_model(ticker, industry):
...
def is_balance_sheet_financial(ticker, industry):
...
def _sanitize_growth_value(raw, industry):
 v = _n(raw)
 if v is None:
  return (None, False)
 bound = SUSPECT_REV_GROWTH_FINANCIAL if is_financial_industry(industry) else SUSPECT_REV_GROWTH_GENERAL
 return (None, True) if v < -100 or v > bound else (v, False)

def sanitize_revenue_growth(row):
 return _sanitize_growth_value(row.get('revenueGrowthTTM'), row.get('industry'))

def sanitize_quarterly_rev_growth(row):
 return _sanitize_growth_value(row.get('revenueGrowthQuarterly'), row.get('industry'))

def sanitize_score_row(row):
 out = dict(row)
 ...
 out['trailingPE'] = _bounded(row.get('trailingPE'), _JS_MIN_VALUE, 1000)
 out['forwardPE'] = _bounded(row.get('forwardPE'), _JS_MIN_VALUE, 1000)
 out['dividendYieldPercent'] = _bounded(row.get('dividendYieldPercent'), 0, 25)
 out['ytdReturn'] = _bounded(row.get('ytdReturn'), -100, 1000)
 ...
 out['evToEbitda'] = _bounded(row.get('evToEbitda'), _JS_MIN_VALUE, 1000)
 out['interestCoverage'] = _bounded(row.get('interestCoverage'), -10000, 10000)
...
def compute_breakdown(input_row):
 ...
 range_pos = _n(_clamp_fraction(row.get('rangePosition'))) if row.get('rangePosition') is not None else None
 ...
 if not financial:
  if fcf is not None and fcf < 0:
   ...
   earnings_quality = 1 if conversion > EQ_CONVERSION_STRONG else -1 if conversion < EQ_CONVERSION_WEAK else 0
 leverage = 0
 ...
 revenue_growth = (1 if rev_growth > 10 else -1 if rev_growth < 0 else 0) if rev_growth is not None else 0
 rev_q = sanitize_quarterly_rev_growth(row)[0]
 revenue_acceleration = 0
 if rev_growth is not None and rev_q is not None:
  revenue_acceleration = 1 if rev_q > rev_growth + REV_ACCEL_THRESHOLD_PP else -1 if rev_q < rev_growth - REV_ACCEL_THRESHOLD_PP else 0
 fcf_yield_level = (1 if fcf > 5 else -1 if fcf < 2 else 0) if fcf is not None and (not financial) else 0
 op_ttm = _n(row.get('operatingMarginTTM'))
 op_5y = _n(row.get('operatingMargin5Y'))
 margin_inflection = 0
 if op_ttm is not None and op_5y is not None:
  margin_inflection = 1 if op_ttm > op_5y + MARGIN_INFLECTION_PP else -1 if op_ttm < op_5y - MARGIN_INFLECTION_PP else 0
 pe_compression = 0
 if pe is not None and fwd_pe is not None:
  ...
  if pe_compression == 1 and is_cyclical_industry(row.get('industry')):
   pe_compression = 0
 valuation = (1 if ev_ebitda < 15 else -1 if ev_ebitda > 25 else 0) if ev_ebitda is not None and (not financial) else 0
 dividend_coverage = 0
 if div_yield is not None and div_yield > 0 and (not financial):
  if fcf is not None:
   dividend_coverage = 1 if fcf > div_yield else -1
 price_position = (1 if range_pos < 0.4 else -1 if range_pos > 0.9 else 0) if range_pos is not None else 0
 ytd_momentum = (1 if ytd > 0 else -1 if ytd < 0 else 0) if ytd is not None else 0
 dividend_yield = (1 if div_yield > 1.5 else 0) if div_yield is not None and div_yield > 0 else 0
 return {'earningsQuality': earnings_quality, 'leverage': leverage, 'revenueGrowth': revenue_growth, 'revenueAcceleration': revenue_acceleration, 'fcfYieldLevel': fcf_yield_level, 'marginInflection': margin_inflection, 'peCompression': pe_compression, 'valuation': valuation, 'dividendCoverage': dividend_coverage, 'pricePosition': price_position, 'ytdMomentum': ytd_momentum, 'dividendYield': dividend_yield}

def compute_scores(breakdown):
 strength = 0
 risk = 0
...
def compute_coverage(row):
 ...
 range_pos = _n(_clamp_fraction(row.get('rangePosition'))) if row.get('rangePosition') is not None else None
 financial = is_balance_sheet_financial(row.get('ticker'), row.get('industry'))
 per_criterion = {'earningsQuality': (not financial, fcf is not None and (fcf < 0 or pe is not None)), 'leverage': (not financial and (not is_reit_industry(row.get('industry'))), de is not None), 'revenueGrowth': (True, rev is not None), 'revenueAcceleration': (True, rev is not None and sanitize_quarterly_rev_growth(row)[0] is not None), 'fcfYieldLevel': (not financial, fcf is not None), 'marginInflection': (True, _n(row.get('operatingMarginTTM')) is not None and _n(row.get('operatingMargin5Y')) is not None), 'peCompression': (True, pe is not None and fwd_pe is not None), 'valuation': (not financial, ev_ebitda is not None), 'dividendCoverage': (not financial, div_yield is not None and (div_yield <= 0 or fcf is not None)), 'pricePosition': (True, range_pos is not None), 'ytdMomentum': (True, ytd is not None), 'dividendYield': (True, div_yield is not None)}
 covered = 0
 applicable = 0
 for key in CRITERION_KEYS:
  ...
  if is_covered:
   covered += 1
 return {'covered': covered, 'applicable': applicable, 'fraction': 0 if applicable == 0 else covered / applicable}

def has_insufficient_data(row, coverage=None):
 ...
 if not financial and _n(row.get('fcfYieldPercent')) is None:
  return True
 if not financial and (not is_reit_industry(row.get('industry'))) and (_n(row.get('debtToEquity')) is None):
  return True
 return False

def is_benign_earnings_quality(row):
 fcf = _n(row.get('fcfYieldPercent'))
 rev = sanitize_revenue_growth(row)[0]
 return fcf is not None and fcf >= BENIGN_EQ_MIN_FCF_YIELD and (rev is not None) and (rev > BENIGN_EQ_MIN_REV_GROWTH)

def is_critical_earnings_quality(row):
 fcf = _n(row.get('fcfYieldPercent'))
 pe = _n(row.get('trailingPE'))
 if fcf is None:
  return False
...
def is_serviceable_leverage(row):
 ic = _n(row.get('interestCoverage'))
 return ic is not None and ic >= STRONG_INTEREST_COVERAGE

def is_disqualified(breakdown, row):
 eq = breakdown['earningsQuality'] == -1 and is_critical_earnings_quality(row) and (not is_benign_earnings_quality(row))
...
def tier_for(strength_score, risk_score, flags):
...
def score_row(raw_row):
 ...
 flags = {'disqualified': is_disqualified(breakdown, row), 'cyclical': is_cyclical_industry(row.get('industry')), 'crowding': is_crowded(row), 'benignEarningsQuality': breakdown['earningsQuality'] == -1 and is_benign_earnings_quality(row), 'suspectRevenueGrowth': sanitize_revenue_growth(raw_row)[1] or sanitize_quarterly_rev_growth(raw_row)[1], 'insufficientData': has_insufficient_data(row, coverage), 'valueTrap': is_value_trap(row), 'peakCycle': is_peak_cycle(row), 'serviceableLeverage': breakdown['leverage'] == -1 and is_serviceable_leverage(row), 'softEarningsQuality': breakdown['earningsQuality'] == -1 and (not is_critical_earnings_quality(row)) and (not is_benign_earnings_quality(row))}
 return {'breakdown': breakdown, 'strengthScore': strength, 'riskScore': risk, 'coverage': coverage, 'flags': flags, 'tier': tier_for(strength, risk, flags)}
def _resolve(enum_cls, candidates):
 for name in candidates:
  value = getattr(enum_cls, name, None)
  if value is not None:
   return int(value)
 raise ValueError(f'industry_map (P-B): none of {candidates} exist on {enum_cls.__name__}; fix the candidate list against the QC datalib and rerun')
...
CYCLICAL_SECTORS = {_resolve(MorningstarSectorCode, ['BASIC_MATERIALS']), _resolve(MorningstarSectorCode, ['ENERGY'])}
CYCLICAL_INDUSTRY_GROUPS = {_resolve(MorningstarIndustryGroupCode, ['SEMICONDUCTORS']), _resolve(MorningstarIndustryGroupCode, ['VEHICLES_AND_PARTS'])}
...
MARGIN_WINDOW = 60
HISTORY_BARS = 260
MISS_FIELDS = 'trailingPE forwardPE fcfYieldPercent evToEbitda revenueGrowthTTM revenueGrowthQuarterly operatingMarginTTM operatingMargin5Y interestCoverage debtToEquity ytdReturn rangePosition'.split()
...
class QR009SmallcapV5(QCAlgorithm):
 ...
 def initialize(self):
  ...
  self.schedule.on(self.date_rules.month_start('SPY'), self.time_rules.after_market_open('SPY', 90), self._step)
  self._sel_month = -1
 ...
 def _select(self, fundamentals):
  ...
  for (_, f) in rows:
   ...
   if len(top) == RANK_HI:
    break
  band = []
  for f in top[RANK_LO:]:
   dv = _walk(f, 'dollar_volume')
   if dv is None or dv <= MIN_DOLLAR_VOLUME:
    self._dv_excluded += 1
    continue
   band.append(f)
  self._u_eligible = [f.symbol for f in band]
  self._u_top100 = [f.symbol for f in band[:TOP_100]]
  self._pending = {f.symbol: self._row(f) for f in band}
  held = set()
  if self._books:
   for book in self._books.values():
    held.update(book['members'])
  self._universe_cache = self._u_eligible + [s for s in held if s not in set(self._u_eligible)]
  return self._universe_cache
 ...
 def _row(self, f):
  ...
  return {'ticker': f.symbol.value, 'industry': industry_label(f), 'marketCap': _walk(f, 'market_cap'), 'trailingPE': pos('valuation_ratios.pe_ratio'), 'forwardPE': pos('valuation_ratios.forward_pe_ratio'), 'dividendYieldPercent': div if div is not None else 0.0, 'fcfYieldPercent': x100('valuation_ratios.fcf_yield'), 'evToEbitda': pos('valuation_ratios.ev_to_ebitda'), 'revenueGrowthTTM': x100('operation_ratios.revenue_growth.one_year'), 'revenueGrowthQuarterly': x100('operation_ratios.revenue_growth.three_months'), 'operatingMarginTTM': x100('operation_ratios.operation_margin.one_year'), 'operatingMargin5Y': sum(margins) / len(margins) if len(margins) >= MARGIN_MIN_OBS else None, 'interestCoverage': _walk(f, 'operation_ratios.interest_coverage.one_year'), 'debtToEquity': _walk(f, 'operation_ratios.total_debt_equity_ratio.three_months'), 'ytdReturn': None, 'rangePosition': None, 'currentPrice': None}
 ...
 def _step(self):
  ...
  if not first:
   for book in self._books.values():
    self._resolve(book)
   for (sym, series) in ((self._spy, self._spy_series), (self._iwm, self._iwm_series)):
    p = self._price(sym)
    pv = self._bm_prev.get(sym)
    series.append(p / pv - 1 if p is not None and pv is not None else None)
    if p is not None:
     self._bm_prev[sym] = p
  else:
   for sym in (self._spy, self._iwm):
    self._bm_prev[sym] = self._price(sym)
  ...
  self._rebalance(self._books['ew_top100'], sorted(self._u_top100, key=lambda s: s.value))
  ...
  record['skips'] = self._entry_skips
  record['dvx'] = self._dv_excluded
 ...
 def _score_universe(self):
  ...
  record = {'breadth': len(strong), 'insuf': n_insufficient, 'susp': n_suspect, 'miss': miss, 'cand': sorted((s.value for s in strong)), 'buckets': cand_industries}
 ...
 def _net(self, book, bps):
  return [apply_cost(g, book['to'][m], bps) if g is not None else None for (m, g) in enumerate(book['gross'])]
 ...
 def on_end_of_algorithm(self):
  ...
  def stats_line(name, series, lo, hi):
   ...
   return f'{name} {lo}-{hi}: sharpe={st.sharpe:.3f} cagr={st.cagr:.2%} mdd={st.max_drawdown:.2%} n={st.observations}'
  ...
  if not any((v is None for v in primary_cand)) and len(primary_cand) >= 2:
   ...
   if any((v is None for v in ew100_part)):
    ...
    if not ctrl_sharpes:
     ...
     detail = f'cand={cand_sharpe:.3f} ctrl_p75={p75:.3f} ew100={ew100_sharpe:.3f} ctrl_dropped={dropped}'
  else:
   detail = f'candidate has {sum((1 for v in primary_cand if v is None))} void months'
  self.debug(f'PRIMARY VERDICT: {verdict} ({detail})')
  self.debug(stats_line('CAND', cand_net, lo, hi))
  self.debug(stats_line('CANDx2', cand_stress, lo, hi))
  self.debug(stats_line('EW100', ew100_net, lo, hi))
  self.debug(stats_line('EWU', ewu_net, lo, hi))
  self.debug(stats_line('SPY', self._spy_series, lo, hi))
  self.debug(stats_line('IWM', self._iwm_series, lo, hi))
  for (sub_lo, sub_hi) in SUBPERIODS:
   self.debug(stats_line('CAND', cand_net, sub_lo, sub_hi))
  self.debug(stats_line('CAND OOS', cand_net, 2023, 2026))
  self.debug(stats_line('EW100 OOS', ew100_net, 2023, 2026))
  self.debug(stats_line('SPY OOS', self._spy_series, 2023, 2026))
  self.debug(stats_line('IWM OOS', self._iwm_series, 2023, 2026))
  ...
  payload = {'experiment': 'QR-009', 'scoring_version': 5, 'seed_base': SEED_BASE, 'base_bps': BASE_BPS, 'dates': self._dates, 'spy': self._spy_series, 'iwm': self._iwm_series, 'books': {name: {'gross': b['gross'], 'to': b['to'], 'invalid': b['invalid']} for (name, b) in self._books.items()}, 'records': self._records, 'entry_skips': self._entry_skips}
  self.object_store.save('qr009/results.json', json.dumps(payload))
  self.debug('saved qr009/results.json')
