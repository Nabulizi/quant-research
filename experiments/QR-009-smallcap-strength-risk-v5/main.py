# QR-009: Strength/Risk v5 in the small-cap band (ranks 501-1500).
...
#   universe = ranks 501-1500 with a $2M/day dollar-volume floor (excluded
#   count logged); costs 25/50 bps; seed base QR009; IWM reported as an
#   additional benchmark; ObjectStore key qr009/results.json. All scoring,
#   accounting, execution, and control mechanics are byte-identical.
#
# QC project manifest: main.py (this file), industry_map.py (from
# experiments/QR-008-strength-risk-v5/), scoring_v5.py + core.py (verbatim
# from quant_research/).
#
...
#   - Full series saved to ObjectStore key "qr009/results.json"; the Debug
...
from industry_map import industry_label
...
class QR009SmallcapV5(QCAlgorithm):
    ...
    def _select(self, fundamentals):
        ...
        # Dedupe (QC feed emitted duplicates, seen 2014-07 in QR-008), rank as
        # QR-008 does, take the 501-1500 band, then apply the liquidity floor.
        top = []
        seen = set()
        for _, f in rows:
            if f.symbol in seen:
    ...
    def _step(self):
        ...
        if not first:
            for book in self._books.values():
                self._resolve(book)
            for sym, series in ((self._spy, self._spy_series),
                                (self._iwm, self._iwm_series)):
                p = self._price(sym)
    ...
    def _score_universe(self):
        ...
        record = {
            "breadth": len(strong),
            "insuf": n_insufficient,
            "susp": n_suspect,
            "miss": miss,
            "cand": sorted(s.value for s in strong),
            "buckets": cand_industries,
        }
